...
import { Plus, CircleNotch, Trash, MagnifyingGlass, X } from "@phosphor-icons/react";
...
export default function ThreadContainer({
  ...
  function sortThreadsWithPinned(list = [], pinnedSlugs = []) {
    return [...list].sort((a, b) => {
      const aPinned = pinnedSlugs.includes(a.slug);
      const bPinned = pinnedSlugs.includes(b.slug);
      if (aPinned !== bPinned) return aPinned ? -1 : 1;
      return new Date(b.lastUpdatedAt).getTime() - new Date(a.lastUpdatedAt).getTime();
    });
  }

  function persistPinned(slugs = []) {
    window.localStorage.setItem(pinStorageKey, JSON.stringify(slugs));
  }
  ...
  useEffect(() => {
    ...
    if (Array.isArray(storedPinned)) {
      setPinnedThreadSlugs(storedPinned);
    }
  }, [pinStorageKey]);
  ...
  useEffect(() => {
    async function fetchThreads() {
      ...
      setLoading(false);
      setThreads(sortThreadsWithPinned(threads, pinnedThreadSlugs));
    }
    fetchThreads();
  ...
  function togglePin(threadSlugToToggle) {
    setPinnedThreadSlugs((prev) => {
      const next = prev.includes(threadSlugToToggle)
        ? prev.filter((slug) => slug !== threadSlugToToggle)
        : [threadSlugToToggle, ...prev];
      persistPinned(next);
      return next;
    });
  }
    ...
    <div className="flex flex-col" role="list" aria-label="Threads">
        ...
        <div className="relative">
          ...
          {!!searchQuery && (
            <button
              ...
              className="absolute right-1 top-1/2 -translate-y-1/2 text-theme-text-secondary hover:text-theme-text-primary p-1 border-none bg-transparent"
      ...
      {filteredThreads.map((thread, i) => (
        <ThreadItem
          ...
          pinned={pinnedThreadSlugs.includes(thread.slug)}
          thread={thread}
          hasNext={i !== filteredThreads.length - 1 || isVirtualThread}
        />
      ))}
      {filteredThreads.length === 0 && (
        <p className="px-4 py-2 text-xs text-theme-text-secondary">
