...
import { formatRelativeTimeAgo } from "@/utils/lawyerRevizorro/threadMeta";
...
export default function LawyerRevizorroHomeThreadHistory({
  workspace: workspaceProp = null,
  activeThreadSlug = null,
}) {
  const { t, i18n } = useTranslation("lawyerRevizorro");
  ...
  const pinStorageKey = `lawyerRevizorro_home_pinned_threads_${workspace?.slug || "default"}`;
  const pendingDeleteStorageKey = `lawyerRevizorro_home_pending_deletes_${workspace?.slug || "default"}`;
  ...
  }, [workspace?.slug, pinnedThreadSlugs]);
  ...
  if (loading) {
    return (
      <div className="lawyerRevizorro-home-thread-history mt-6 w-full">
        <Skeleton.default height={52} count={3} className="mb-2" />
      </div>
    );
  }
  ...
  return (
    <nav
      className="lawyerRevizorro-home-thread-history mt-6 w-full"
    ...
    >
      ...
      {pendingDeletes.length > 0 && (
        <ul className="lawyerRevizorro-home-thread-history__pending mb-2 flex flex-col gap-1">
      ...
      <ul className="lawyerRevizorro-home-thread-history__list">
          ...
          return (
            <li key={thread.slug}>
              <div className="flex items-center gap-1.5">
                <button
                  type="button"
                  className={`lawyerRevizorro-home-thread-history__item flex-1${isActive ? " lawyerRevizorro-home-thread-history__item--active" : ""}`}
                  onClick={() => {
                    if (isActive) {
                      startNewConversation(navigate);
                      return;
                    }
                    navigate(paths.lawyerRevizorro.thread(workspace.slug, thread.slug));
                  }}
                >
                  {renamingSlug === thread.slug ? (
                    ...
                    <>
                      <span className="lawyerRevizorro-home-thread-history__title">
                        {pinned ? "📌 " : ""}
                        {thread.name}
                      </span>
                      {ago && (
                        <span className="lawyerRevizorro-home-thread-history__meta">
