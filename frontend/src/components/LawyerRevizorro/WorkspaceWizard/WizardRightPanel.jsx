...
export default function WizardRightPanel({ onUseTemplate }) {
  ...
  function handleDocDownload(doc) {
    const content = `lawyer-revizorro — ${doc.name}\nSize: ${doc.size}\nGenerated: ${new Date().toLocaleDateString()}`;
  ...
  return (
    <>
      <h3 className="lawyerRevizorro-document-panel__eyebrow mb-3">
        {t("admin.supportingDocs")}
      </h3>
      <ul className="list-none p-0 m-0 mb-6">
        {SUPPORTING_DOCUMENTS.map((doc) => (
          <li key={doc.name} className="lawyerRevizorro-doc-list-item">
            {doc.type === "pdf" ? (
      ...
      {WORKSPACE_TEMPLATES.map((tpl) => (
        <div key={tpl.id} className="lawyerRevizorro-template-card">
          <p className="lawyerRevizorro-template-card__title">{tpl.title}</p>
          <p className="lawyerRevizorro-template-card__desc">{tpl.description}</p>
          <button
            type="button"
            className="lawyerRevizorro-template-card__use"
      ...
      <button type="button" onClick={() => navigate("/settings/workspaces")} className="lawyerRevizorro-suite-back mt-4 w-full justify-center">
