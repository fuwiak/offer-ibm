...
import { QUOTE_STEPS, advanceQuoteDraft, updateQuoteLines } from "@/utils/lawyerRevizorro/quoteFlow";
import { LAWYER_REVIZORRO_PRODUCTS } from "@/utils/lawyerRevizorro/pricing";
...
export default function QuoteStepper() {
  ...
  const { quoteDraft, setQuoteDraft, setQuotePdfUrl, setDocumentPanelView } = useLawyerRevizorro();
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      const result = await LawyerRevizorro.generateQuotePdf({
      ...
      setPdfMeta(result);
      const apiUrl = LawyerRevizorro.quotePdfDownloadUrl(result.storageFilename);
    ...
    <div className="space-y-2">
      ...
      {lines.map((line, i) => (
        <div key={i}>
          ...
          >
            {LAWYER_REVIZORRO_PRODUCTS.map((p) => (
...
function ShareStep({ reference }) {
  ...
  const url = reference
    ? `${window.location.origin}/share/${btoa(`lawyerRevizorro-${reference}`).slice(0, 20)}`
    : null;
