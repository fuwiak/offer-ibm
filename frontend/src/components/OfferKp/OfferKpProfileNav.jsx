...
export default function OfferKpProfileNav() {
  ...
  const profilePath = paths.offerKp.profile();
  const usersPath = paths.offerKp.users();
  ...
  if (!showProfileGroup) {
    return (
      <Link
        to={profilePath}
        className={`offerKp-nav-item ${profileActive ? "offerKp-nav-item--active" : ""}`}
      >
  ...
  return (
    <div className="offerKp-nav-group">
      <button
        type="button"
        className={`offerKp-nav-item w-full ${groupActive ? "offerKp-nav-item--active" : ""}`}
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      >
        <span className="flex items-center gap-2">
      ...
      {open && (
        <nav className="offerKp-nav-sub" aria-label={t("admin.profile")}>
          {subItems.map(({ key, icon: Icon, path, label, active }) => (
            <Link
              key={key}
              to={path}
              className={`offerKp-nav-subitem ${active ? "offerKp-nav-subitem--active" : ""}`}
            >
