...
import {
  LAWYER_REVIZORRO_ANTHROPIC_FALLBACK_MODELS,
  LAWYER_REVIZORRO_DEFAULT_ANTHROPIC_MODEL,
} from "@/utils/lawyerRevizorro/anthropicModels";
import { LAWYER_REVIZORRO_OPENROUTER_PROVIDER } from "@/utils/lawyerRevizorro/llmProviders";

async function loadAnthropicModels() {
  const { models = [] } = await System.customModels(LAWYER_REVIZORRO_OPENROUTER_PROVIDER);
  if (models?.length > 0) return models;
  return LAWYER_REVIZORRO_ANTHROPIC_FALLBACK_MODELS;
}

export default function LawyerRevizorroAnthropicModelPicker({ workspaceSlug }) {
  const { t } = useTranslation();
  const [open, setOpen] = useState(false);
  const [models, setModels] = useState(LAWYER_REVIZORRO_ANTHROPIC_FALLBACK_MODELS);
  const [selectedModel, setSelectedModel] = useState(LAWYER_REVIZORRO_DEFAULT_ANTHROPIC_MODEL);
  ...
  const refresh = useCallback(async () => {
    ...
    setSelectedModel(known ? current : LAWYER_REVIZORRO_DEFAULT_ANTHROPIC_MODEL);
  }, [workspaceSlug]);
  ...
  async function handleSelect(modelId) {
    ...
    try {
      const { message } = await Workspace.update(workspaceSlug, {
        chatProvider: LAWYER_REVIZORRO_OPENROUTER_PROVIDER,
        chatModel: modelId,
      });
