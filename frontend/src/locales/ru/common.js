...
const TRANSLATIONS = {
  onboarding: {
    ...
    llm: {
      ...
      description:
        "OfferKP может работать с различными провайдерами LLM. Этот сервис будет обеспечивать обработку чата.",
    },
    userSetup: {
    ...
    firstRunSetup: {
      title: "Первый запуск",
      subtitle: "Создайте учётную запись администратора, чтобы начать.",
      username: "Имя пользователя",
      password: "Пароль",
      confirmPassword: "Подтвердите пароль",
      passwordPlaceholder: "Минимум 8 символов",
      confirmPasswordPlaceholder: "Повторите пароль",
      submit: "Создать учётную запись администратора",
      submitting: "Создание учётной записи...",
      passwordsMismatch: "Пароли не совпадают.",
      passwordTooShort: "Пароль должен содержать не менее 8 символов.",
      createFailed: "Не удалось создать учётную запись администратора.",
      createSuccess: "Учётная запись администратора создана. Добро пожаловать!",
      error: "Произошла ошибка: {{message}}",
    },
    survey: {
      title: "Добро пожаловать в OfferKP",
      description:
        "Помогите нам сделать OfferKP, созданным с учётом ваших потребностей (необязательно).",
      email: "Какой у вас адрес электронной почты?",
      useCase: "Для чего вы будете использовать OfferKP?",
      useCaseWork: "Для работы",
      useCasePersonal: "Для личного использования",
      useCaseOther: "Другое",
      comment: "Откуда вы узнали о OfferKP?",
      commentPlaceholder:
  ...
  login: {
    "multi-user": {
      welcome: "Добро пожаловать в OfferKP",
    ...
    "sign-in":
      "Войдите, чтобы обрабатывать заявки на крепёж и формировать коммерческие предложения.",
    "password-reset": {
      title: "Сброс пароля",
      description:
  ...
  agent: {
    ...
    settings: {
      ...
      "intelligent-skill-selection": {
        ...
        description:
          "Позволяет использовать неограниченное количество инструментов и сократить использование токенов до 80% на запрос – OfferKP автоматически выбирает наиболее подходящие навыки для каждого запроса.",
        "max-tools": {
  ...
  api: {
    title: "API ключи",
    description:
      "API ключи позволяют владельцу программно получать доступ к этому экземпляру OfferKP и управлять им.",
    ...
    modal: {
      ...
      helper:
        "После создания API-ключ можно использовать для программного доступа к этому экземпляру OfferKP и его настройки.",
      name: {
        label: "Название",
  ...
  llm: {
    ...
    description:
      "Это учетные данные и настройки для вашего предпочтительного поставщика чата и встраивания LLM. Важно, чтобы эти ключи были актуальными и правильными, иначе OfferKP не будет работать должным образом.",
  ...
  transcription: {
    ...
    "warn-start":
      "Использование локальной модели whisper на машинах с ограниченной оперативной памятью или процессором может привести к зависанию OfferKP при обработке медиафайлов.",
    "warn-recommend":
  ...
  embedding: {
    ...
    "desc-end":
      "Встраивание - это процесс превращения текста в векторы. Эти учетные данные необходимы для превращения ваших файлов и подсказок в формат, который OfferKP может использовать для обработки.",
    provider: {
  ...
  vector: {
    ...
    description:
      "Это учетные данные и настройки для того, как будет функционировать ваш экземпляр OfferKP. Важно, чтобы эти ключи были актуальными и правильными.",
    provider: {
  ...
  privacy: {
    ...
    description:
      "Это ваша конфигурация для того, как подключенные сторонние поставщики и OfferKP обрабатывают ваши данные.",
  ...
  connectors: {
    ...
    upload: {
      ...
      "privacy-notice":
        "Эти файлы будут загружены в процессор документов на этом экземпляре OfferKP. Файлы не отправляются и не передаются третьим лицам.",
    },
    pinning: {
      what_pinning: "Что такое закрепление документа?",
      pin_explained_block1:
        "Когда вы <b>закрепляете</b> документ в OfferKP, мы вставляем всё его содержимое в окно запроса, чтобы LLM мог полностью его понять.",
      pin_explained_block2:
        "Это работает лучше всего с <b>моделями с большим контекстом</b> или небольшими файлами, критичными для базы знаний.",
      pin_explained_block3:
        "Если по умолчанию ответы OfferKP вас не удовлетворяют, закрепление — отличный способ получить более качественные ответы одним кликом.",
      accept: "Хорошо, понял",
    },
    watching: {
      what_watching: "Что делает функция наблюдения за документом?",
      watch_explained_block1:
        "Когда вы <b>наблюдаете</b> за документом в OfferKP, мы <i>автоматически</i> синхронизируем его содержимое с оригинальным источником через регулярные интервалы. Это автоматически обновляет содержимое во всех рабочих пространствах, где используется этот файл.",
  ...
  chat_window: {
    ...
    copy: "Копировать",
    regenerate: "Повторить запрос",
    regenerate_response: "Перезапустить последний ответ",
    run_again: "Запустить снова",
    run_again_tooltip:
      "Пересчитать тред с этого сообщения (удалит последующие ответы и пересоберёт сводку КП)",
    good_response: "Хороший ответ",
    more_actions: "Больше действий",
    fork: "Вилка",
    delete: "Удалить",
    cancel: "Отменить",
    edit_prompt: "Редактировать сообщение",
    ...
    submit: "Отправить",
    edit_info_user:
      '"Отправить" пересчитает тред с этого сообщения (удалит последующие ответы и пересоберёт сводку КП). "Сохранить" обновит только текст.',
    edit_info_assistant:
  ...
  customization: {
    interface: {
      ...
      description:
        "Настройте свои предпочтения пользовательского интерфейса для OfferKP.",
    },
    branding: {
      title: "Брендинг и создание продуктов с собственной меткой.",
      description:
        "Настройте свою версию OfferKP с использованием собственных брендинговых элементов.",
    },
    chat: {
      title: "Чат",
      description: "Настройте свои предпочтения для чата с OfferKP.",
      auto_submit: {
    ...
    items: {
      ...
      "display-language": {
        ...
        description:
          "Выберите предпочитаемый язык для отображения пользовательского интерфейса OfferKP – когда доступны переводы.",
      },
      logo: {
    ...
    publish: {
      ...
      generic: {
        unauthenticated: {
          ...
          description:
            "Для публикации материалов необходимо сначала пройти аутентификацию в сообществе OfferKP.",
  ...
  security: {
    ...
    password: {
      ...
      description:
        "Защитите ваш экземпляр OfferKP паролем. Если вы забудете его, метода восстановления не существует, поэтому убедитесь, что вы сохранили этот пароль.",
  ...
  telegram: {
    ...
    description:
      "Подключите свою инстанцию OfferKP к Telegram, чтобы вы могли общаться со своими рабочими пространствами с любого устройства.",
    setup: {
      step1: {
