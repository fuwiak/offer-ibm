...
const Workspace = {
  ...
  /**
   * Parse a file while receiving OCR progress over SSE.
   * Calls handlers as events arrive so the UI can show page-by-page progress
   * for large scanned documents instead of a single long "Uploading..." wait.
   * @param {string} slug
   * @param {FormData} formData
   * @param {{
   *   onStage?: (stage: string) => void,
   *   onPage?: (info: {pageNumber: number, totalPages: number}) => void,
   *   onComplete?: (files: object[]) => void,
   *   onError?: (error: string) => void,
   * }} handlers
   * @returns {Promise<void>}
   */
  parseFileStream: async function (slug, formData, handlers = {}) {
    const { onStage, onPage, onComplete, onError } = handlers;
    const ctrl = new AbortController();
    let settled = false;

    try {
      await fetchEventSource(`${API_BASE}/workspace/${slug}/parse-stream`, {
        method: "POST",
        body: formData,
        headers: baseHeaders(),
        signal: ctrl.signal,
        ...
        async onopen(response) {
          ...
          settled = true;
          onError?.(`Upload failed (code ${response.status}).`);
          ctrl.abort();
        ...
        onmessage(msg) {
          ...
          switch (event.type) {
            ...
            case "error":
              settled = true;
              onError?.(event.error || "Processing error.");
              ctrl.abort();
              break;
            default:
              break;
        ...
        onerror(err) {
          // fetchEventSource calls onerror on normal stream close too; only
          // surface a real error if we never received a terminal event.
          if (settled) throw err; // stop retrying
          settled = true;
          onError?.(err?.message || "Connection error during upload.");
          throw err;
        },
      });
    } catch (_) {
      // Abort after a terminal event throws here — already handled above.
      if (!settled) onError?.("Connection error during upload.");
    }
  },
