...
const System = {
  ...
  emergencyPasswordReset: async function ({
    username,
    newPassword,
    confirmPassword,
    masterKey,
  }) {
    return await fetch(`${API_BASE}/system/force-admin-reset`, {
      method: "POST",
      headers: jsonHeaders(),
      body: JSON.stringify({
        username: String(username).trim().toLowerCase(),
  ...
  setupStatus: async () => {
    return await fetch(`${API_BASE}/system/setup-status`)
      .then((res) => res.json())
      .catch(() => ({
        hasUsers: false,
        userCount: 0,
  ...
  fetchLogo: async function () {
    ...
    try {
      ...
      return { isCustomLogo, logoURL: URL.createObjectURL(blob) };
    } catch (e) {
      console.warn("Logo fetch unavailable, using bundled default:", e.message);
      return { isCustomLogo: false, logoURL: null };
    }
  },
