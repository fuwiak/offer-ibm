...
const Admin = {
  ...
  addUserToWorkspace: async (workspaceId, userId) => {
    return await fetch(`${API_BASE}/admin/workspaces/${workspaceId}/users`, {
      method: "POST",
      headers: baseHeaders(),
  ...
  removeUserFromWorkspace: async (workspaceId, userId) => {
    return await fetch(
      `${API_BASE}/admin/workspaces/${workspaceId}/users/${userId}`,
      {
        method: "DELETE",
        headers: baseHeaders(),
      }
  ...
  updateWorkspaceMembership: async (workspaceId, userId, { suspended }) => {
    return await fetch(
      `${API_BASE}/admin/workspaces/${workspaceId}/users/${userId}/membership`,
      {
        method: "POST",
        headers: baseHeaders(),
  ...
  // ── System skills (admin-managed slash command presets for all users) ──
  getSystemSkills: async function () {
    return fetch(`${API_BASE}/admin/system-skills`, {
      method: "GET",
      headers: baseHeaders(),
    })
      .then((res) => {
        if (!res.ok) throw new Error("Could not fetch system skills.");
        return res.json();
      })
  ...
  createSystemSkill: async function (skillData = {}) {
    return fetch(`${API_BASE}/admin/system-skills`, {
      method: "POST",
      headers: baseHeaders(),
      ...
      .then((res) => res.json())
      .then((data) => ({ skill: data.skill ?? null, error: data.message ?? null }))
      .catch((e) => ({ skill: null, error: e.message }));
  },

  updateSystemSkill: async function (skillId, skillData = {}) {
    return fetch(`${API_BASE}/admin/system-skills/${skillId}`, {
      method: "POST",
      headers: baseHeaders(),
      body: JSON.stringify(skillData),
    })
  ...
  deleteSystemSkill: async function (skillId) {
    return fetch(`${API_BASE}/admin/system-skills/${skillId}`, {
      method: "DELETE",
      headers: baseHeaders(),
    })
