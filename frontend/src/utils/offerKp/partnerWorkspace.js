...
export const PARTNER_WORKSPACE_SLUG =
  import.meta.env.VITE_OFFER_KP_PARTNER_WORKSPACE || "offerKp-partner";

/**
 * Resolves the workspace used for partner chat from the home screen.
 * Prefers configured slug, then any offerKp-* workspace, then last visited.
 */
export async function resolvePartnerWorkspace(
  fallbackName = "offer-kp Partner",
  preferredSlug = null
) {
  if (preferredSlug) {
  ...
  const offerKpWs = all.find((w) => w.slug?.startsWith("offerKp"));
  if (offerKpWs) return offerKpWs;
