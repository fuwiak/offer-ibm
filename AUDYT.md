...
**Вывод: да, помогает — но не как замена fine-tuning/embedding, а как третий слой поверх уже сделанного (§6.1), закрывающий конкретно найденный пробел (систематическая неполнота входных данных, а не нехватка примеров). Не реализовано в этой сессии — статус: рекомендация, ждёт решения, стоит ли делать.**
