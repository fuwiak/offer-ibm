...
const WorkspaceThread = {
  ...
  threadNameFromPrompt: function (prompt = "", maxLength = 60) {
    if (typeof prompt !== "string") return this.defaultName;
    const cleaned = prompt.trim().replace(/\s+/g, " ");
    if (!cleaned) return this.defaultName;
  ...
  autoRenameThread: async function ({
    ...
    const { thread: updatedThread } = await this.update(thread, {
      name: this.threadNameFromPrompt(prompt),
    });
