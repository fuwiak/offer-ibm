...
const SlashCommandPresets = {
  ...
  getUserPresets: async function (userId = null) {
    try {
      const where = !!userId
        ? { OR: [{ userId: Number(userId) }, { userId: null, uid: -1 }] }
        : { userId: null };
      return (
  ...
  // System skills are admin-created presets available to all users (uid = -1).
  getSystemSkills: async function () {
    try {
      return await prisma.slash_command_presets.findMany({
        where: { uid: -1, userId: null },
        orderBy: { createdAt: "asc" },
      });
    } catch (error) {
      console.error("Failed to get system skills", error.message);
      return [];
    }
  },

  createSystemSkill: async function (presetData = {}) {
    try {
      const existing = await prisma.slash_command_presets.findFirst({
        where: { uid: -1, command: String(presetData.command) },
      });
      ...
      console.error("Failed to create system skill", error.message);
      return null;
    }
  },

  updateSystemSkill: async function (presetId = null, presetData = {}) {
    try {
      const existing = await prisma.slash_command_presets.findFirst({
        where: { id: Number(presetId), uid: -1 },
      });
      ...
      console.error("Failed to update system skill", error.message);
      return null;
    }
  },

  deleteSystemSkill: async function (presetId = null) {
    try {
      const existing = await prisma.slash_command_presets.findFirst({
        where: { id: Number(presetId), uid: -1 },
      });
