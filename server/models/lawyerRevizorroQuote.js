...
} = require("../utils/lawyerRevizorro/pricing");
...
const LawyerRevizorroQuote = {
  async create({ userId = null, partnerId = null, lines = [], shipping = 0 }) {
    ...
    const quote = await prisma.lawyerRevizorro_quotes.create({
      data: {
        reference,
        userId,
        partnerId,
  ...
  async getByReference(reference) {
    return prisma.lawyerRevizorro_quotes.findUnique({
      where: { reference },
  ...
  async listForUser(userId, role) {
    if (role === "supplier") {
      return prisma.lawyerRevizorro_quotes.findMany({
  ...
  async getByReferenceForRole(reference, role) {
    if (role === "supplier") {
      return prisma.lawyerRevizorro_quotes.findFirst({
...
module.exports = { LawyerRevizorroQuote };
