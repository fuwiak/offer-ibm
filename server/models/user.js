...
const User = {
  ...
  validations: {
    ...
    username: (newValue = "") => {
      try {
        const username = String(newValue).trim().toLowerCase();
  ...
  /** Login / recovery lookup — exact match, then legacy case-insensitive row. */
  _getForAuth: async function (rawUsername = "") {
    const username = String(rawUsername).trim().toLowerCase();
    if (!username) return null;
    try {
      const exact = await prisma.users.findFirst({ where: { username } });
