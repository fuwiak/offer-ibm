...
const Workspace = {
    ...
    try {
      const workspace = await prisma.workspaces.create({
        data: {
          ...
          slug,
          ...(creatorId ? { createdBy: Number(creatorId) } : {}),
        },
      });
  ...
  getWithUser: async function (user = null, clause = {}) {
    ...
    try {
      ...
      if (!workspace || !canUserAccessWorkspace(user, workspace)) return null;
  ...
  whereWithUser: async function (
    ...
    try {
      ...
      return filterWorkspacesForUser(user, workspaces);
    } catch (error) {
      console.error(error.message);
      return [];
    }
    ...
    try {
      ...
      const userInfo = usersById.map((user) => {
        ...
        return {
          ...
          role: user.role,
          suspended: workspaceUser?.suspended ?? 0,
