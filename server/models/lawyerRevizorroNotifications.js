...
const LawyerRevizorroNotifications = {
  /**
  ...
  async list({ limit = 20, userId = null, role = "admin" } = {}) {
    ...
    const links = await prisma.lawyerRevizorro_share_links.findMany({
    ...
    const quotes = await prisma.lawyerRevizorro_quotes.findMany({
      where: quoteWhere,
...
module.exports = { LawyerRevizorroNotifications };
