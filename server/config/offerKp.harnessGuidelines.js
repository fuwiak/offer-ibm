...
const OFFER_KP_HARNESS_GUIDELINES = [
  {
    ...
    when: "always",
    text:
      "Не копируй дословно наименования из PDF/заявки. В КП используй название, SKU и цену из блоков [Каталог · purolat.com].",
  },
  {
    id: "units-from-catalog",
    when: "always",
    text:
      "Единицу измерения (кг, м, шт, уп) бери из блока каталога или из заявки. Не подставляй «шт», если в заявке или в каталоге указано «кг» или другая единица веса/длины.",
  },
  {
    id: "stock-status-column",
    when: "quote",
    text:
      "Для каждой позиции КП указывай статус: «В наличии», «В наличии частично», «Аналог», «Под заказ», «Нет в наличии», «Требует проверки» — по данным каталога (count/available).",
  },
  {
    id: "analog-explicit",
    when: "quote",
    text:
      "Если точного DIN/ГОСТ нет, подбирай аналог по правилам DIN↔ГОСТ и явно пиши «запрошено → подобрано» с указанием стандарта из каталога.",
  },
  {
    id: "prices-catalog-only",
    when: "always",
    text:
      "Цены только из блоков [Каталог · purolat.com]. Запрещены плейсхолдеры «[цена]», «уточните» и выдуманные суммы.",
  },
  {
    id: "docx-nonempty",
    when: "quote",
    text:
      "При создании DOCX/PDF через create-docx-file / create-pdf-file передавай полный markdown таблицы КП со всеми строками, количествами, единицами, ценами и статусами — не пустой шаблон.",
  },
];
...
const OFFER_KP_MATCH_PRIORITIES = [
  {
    ...
    deprioritize: ["933"],
    note:
      "Болт ГОСТ 7798 / ISO 4014 → приоритет DIN 931 (болт с неполной резьбой), не DIN 933 (полная резьба).",
  },
  {
    ...
    note:
      "Винт ГОСТ 11738 → DIN 912. По умолчанию выбирай вариант с Н/Р, если в заявке не указано П/Р.",
  },
];
