...
const STOPWORDS = new Set([
  "ustawa","ustawy","ustawie","ustawę","ustawe","ustawą","ustawa",
  "prawo","prawa","prawie","jaki","jaka","jakie","jakich","który","ktory",
  "która","ktora","które","ktore","czym","czy","jest","są","sa","być","byc",
  "oraz","lub","albo","dla","przez","przy","pod","nad","się","sie","jak","co",
  "to","ten","ta","te","tych","tym","tego","oraz","gdzie","kiedy","ile","jakie",
  "proszę","prosze","chcę","chce","potrzebuję","potrzebuje","mam","można","mozna",
  "należy","nalezy","zgodnie","według","wedlug","mnie","mój","moj","moja","moje",
  "informacja","informacje","pytanie","pomoc","witam","dzień","dzien","dobry",
  "this","that","what","which","please","about","with","from","have","does",
]);
...
function parseActTimestamp(act) {
  const candidates = [act?.changeDate, act?.promulgation, act?.announcementDate];
...
async function getEliContext(message, options = {}) {
  ...
  if (!message || typeof message !== "string" || !message.trim()) {
    return {
      contextTexts,
      sources,
      flags: { eliSkippedEmptyMessage: true, eliSearchHitCount: 0, eliDocCount: 0 },
    };
  }
