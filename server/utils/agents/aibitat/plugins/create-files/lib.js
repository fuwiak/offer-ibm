...
class CreateFilesManager {
  ...
  isToolAvailable() {
    ...
    // Railway and other Node.js environments can write files too
    return true;
  }
  ...
  registerOutput(aibitat, type, payload) {
    ...
    // Persist outputs immediately so downloads work before the agent finishes
    // and cleanup does not treat the file as orphaned (chat may still be include:false).
    this.syncOutputsToTrackedChat(aibitat).catch((error) => {
      console.warn(
        `[CreateFilesManager] syncOutputsToTrackedChat failed: ${error.message}`
      );
    });
  }

  /**
   * Writes pending file outputs to the in-flight workspace chat (include:false).
   * @param {object} aibitat
   */
  async syncOutputsToTrackedChat(aibitat) {
    const chatId = aibitat?.trackedChatId;
    const outputs = aibitat?._pendingOutputs;
    if (!chatId || !outputs?.length) return;
    ...
    if (!chat) {
      console.warn(
        `[CreateFilesManager] syncOutputsToTrackedChat - chat ${chatId} not found`
      );
      return;
    }

    const existing = safeJsonParse(chat.response, {
      text: "",
      type: "chat",
      outputs: [],
    });
    const mergedOutputs = [
      ...(existing.outputs || []).filter(
        (o) =>
          !outputs.some(
            (n) => n?.payload?.storageFilename === o?.payload?.storageFilename
          )
      ),
      ...outputs,
    ];
    ...
    await WorkspaceChats.upsert(chatId, {
      ...
      prompt: chat.prompt,
      response: { ...existing, outputs: mergedOutputs },
      user: chat.user_id ? { id: chat.user_id } : null,
      threadId: chat.thread_id,
      include: false,
    });

    console.log(
      `[CreateFilesManager] syncOutputsToTrackedChat - chat ${chatId}, outputs=${mergedOutputs.length}`
    );
  }
  ...
  /**
   * Gets the lawyer-revizorro logo for branding.
