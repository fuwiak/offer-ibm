...
/**
 * Creates a keepalive interval that sends SSE comment pings to prevent
 * proxy / load-balancer timeouts during long post-processing.
 *
 * @param {import("express").Response} response
 * @param {number} intervalMs
 * @returns {NodeJS.Timeout}
 */
function createSseKeepaliveInterval(response, intervalMs = 12_000) {
  return setInterval(() => {
    try {
...
/**
 * Runs `pipeline()` while keeping the SSE connection alive with periodic pings.
 * Clears the interval regardless of whether the pipeline succeeds or throws.
 *
 * @param {import("express").Response} response
 * @param {() => Promise<string>} pipeline
 * @param {number} intervalMs
 * @returns {Promise<string>}
 */
