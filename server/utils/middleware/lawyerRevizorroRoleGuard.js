...
function lawyerRevizorroRoleGuard({ requireAuth = false } = {}) {
  return async (request, response, next) => {
    const user = await userFromSession(request, response);
    response.locals.lawyerRevizorroUser = user;

    if (requireAuth && !user) {
      return response.status(401).json({ error: "Authentication required." });
    }

    response.locals.sanitizeLawyerRevizorroQuote = (quote) => {
...
module.exports = { lawyerRevizorroRoleGuard, stripSupplierFields };
