...
const { OFFER_KP_PRODUCTS } = require("./pricing");
...
async function buildDashboardStats() {
  ...
  const [quotes, partnerRequests] = await Promise.all([
    prisma.offerKp_quotes.findMany({
      include: { lines: true },
