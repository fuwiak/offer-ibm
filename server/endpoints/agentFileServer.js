  ...
  app.get(
    ...
    async (request, response) => {
      try {
        ...
        if (!parsed) {
          console.warn(
            `[agentFileServer] Invalid filename format: ${filename} user=${user?.id ?? "anon"}`
          );
          return response
            .status(400)
        ...
        if (!fileData) {
          console.warn(
            `[agentFileServer] File missing on disk: ${filename} user=${user?.id ?? "anon"}`
          );
          return response
            .status(404)
        ...
        if (!fileSource) {
          if (!user) {
            console.warn(
              `[agentFileServer] No chat reference and unauthenticated: ${filename}`
            );
            return response.status(404).json({
              error: "File not found or access denied",
            });
          }
          // UUID storage names are unguessable; allow authenticated download when the
          // file exists but the chat row is not yet committed (agent still running).
          console.info(
            `[agentFileServer] Serving without DB reference (in-flight agent file): ${filename} user=${user.id}`
          );
          displayFilename = filename;
        } else {
          displayFilename = fileSource.displayFilename || filename;
        }

        // Get mime type and set headers for download
        const mimeType = createFilesLib.getMimeType(`.${parsed.extension}`);
        const safeFilename =
          createFilesLib.sanitizeFilenameForHeader(displayFilename);
