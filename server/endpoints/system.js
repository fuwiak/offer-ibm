...
function systemEndpoints(app) {
  ...
  app.post("/request-token", async (request, response) => {
    try {
      ...
      if (await SystemSettings.isMultiUserMode()) {
        ...
        const { username: rawUsername, password } = reqBody(request);
        ...
        const existingUser = await User._getForAuth(username);
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        const { username: rawUsername, recoveryCodes } = reqBody(request);
        const username = String(rawUsername ?? "")
          .trim()
          .toLowerCase();
  ...
  app.post("/system/emergency-password-reset", async (request, response) => {
    try {
      const masterKey = process.env.PASSWORD_RESET_MASTER_KEY;
      if (!masterKey) {
        response.status(403).json({
          success: false,
          message: "Emergency password reset is not configured.",
        });
        return;
      }
      ...
      if (String(providedKey ?? "") !== masterKey) {
        response.status(403).json({
          success: false,
      ...
      const user = await User._getForAuth(username);
      if (!user) {
        response.status(400).json({
          success: false,
          message:
            userCount === 0
              ? "No users in database. Open /first-run to create admin, or set MULTI_USER_BOOTSTRAP_PASSWORD on the server."
      ...
      if (updateError) {
        response.status(400).json({ success: false, message: updateError });
        return;
      }
      ...
      console.error("Emergency password reset:", error);
      response.status(500).json({ success: false, message: error.message });
    }
  });
  ...
  app.post(
    ...
    async (request, response) => {
      try {
        ...
        if (response.locals.multiUserMode && existingUserCount > 0) {
          response.status(200).json({
            success: false,
        ...
        console.error("[enable-multi-user]", e.message, e);
        response.status(500).json({
          success: false,
          error: e.message || "Failed to enable multi-user mode.",
        });
      }
    }
  );

  app.post("/system/initialize-admin", async (request, response) => {
    try {
      const existingUserCount = await User.count();
      if (existingUserCount > 0) {
        response.status(403).json({
          success: false,
          error:
            "An admin account already exists. Use login or password reset.",
        });
        return;
      }
      ...
      if (error || !user) {
        response.status(400).json({
          success: false,
          error: error || "Failed to create admin account.",
        });
        return;
      }
      ...
      try {
        ...
        console.warn(
          "[initialize-admin] Post-create migration warning:",
      ...
      console.error("[initialize-admin]", e.message, e);
      response.status(500).json({
        success: false,
        error: e.message || "Failed to initialize admin.",
      });
    }
  });

  app.get("/system/setup-status", async (_request, response) => {
    try {
      const userCount = await User.count();
      const multiUserMode = await SystemSettings.isMultiUserMode();
      const usernames =
        userCount > 0 ? (await User.where({}, 20)).map((u) => u.username) : [];
      response.status(200).json({
        hasUsers: userCount > 0,
        userCount,
        multiUserMode,
        usernames,
        needsFirstRun: userCount === 0,
        emergencyResetConfigured: !!process.env.PASSWORD_RESET_MASTER_KEY,
        bootstrapConfigured: !!(
  ...
  // Master-key powered force-reset that ALSO creates the user if missing.
  // Use this when /first-run is unreachable and bootstrap isn't producing the user.
  app.post("/system/force-admin-reset", async (request, response) => {
    try {
      const masterKey = process.env.PASSWORD_RESET_MASTER_KEY;
      if (!masterKey) {
        response.status(403).json({
          success: false,
          message:
            "Set PASSWORD_RESET_MASTER_KEY on the server to use this endpoint.",
        });
        return;
      }
      ...
      if (!username || pwd.length < 8 || pwd !== confirm) {
        response.status(400).json({
          success: false,
          message:
            "username, newPassword (min 8) and matching confirmPassword required.",
        });
        return;
      }
      ...
      const existing = await User._getForAuth(username);
      let user;
      if (existing) {
        user = await prisma.users.update({
          where: { id: existing.id },
          data: { password: hashed, role: "admin", suspended: 0 },
        });
      } else {
      ...
      await EventLogs.logEvent(
        "force_admin_reset",
        { username, created: !existing },
        user.id
      );
      response.status(200).json({
        success: true,
        message: existing
          ? `Password for "${username}" updated. You can log in now.`
          : `Admin "${username}" created. You can log in now.`,
      });
    } catch (e) {
      console.error("[force-admin-reset]", e.message, e);
  ...
  app.get("/system/logo", async function (request, response) {
    try {
      ...
      let { found, buffer, size, mime } = fetchLogo(logoPath);
      ...
      if (!found) {
        ...
        const bundled = fetchLogo(bundledPath);
        if (bundled.found) {
          ({ found, buffer, size, mime } = bundled);
        }
      }
