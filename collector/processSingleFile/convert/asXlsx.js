...
async function asXlsx({
  ...
  try {
    ...
    try {
      workSheetsFromFile = xlsx.parse(fullFilePath);
    } catch (parseErr) {
      console.log(
        `[asXlsx] node-xlsx failed for ${filename} (${parseErr.message}). Trying SmartOCRAgent.`
      );
      workSheetsFromFile = null;
    }
    ...
    if (!workSheetsFromFile || !rawScore.isAcceptable) {
      if (!workSheetsFromFile) {
        console.log(`[asXlsx] Attempting SmartOCRAgent for ${filename}.`);
      } else {
        console.log(
          `[asXlsx] Low-quality extraction for ${filename} (${rawScore.words} words). Trying SmartOCRAgent.`
        );
      }
      const agent = new SmartOCRAgent({ timeout: 60_000, stopOnFirstGood: true });
      const agentResult = await agent.processExcel(fullFilePath);
      if (agentResult && agentResult.score.isAcceptable) {
        console.log(
          `[asXlsx] SmartOCRAgent succeeded via "${agentResult.strategyUsed}" (${agentResult.score.words} words).`
        );
        // Wrap the flat text as a synthetic single-sheet workbook
        workSheetsFromFile = [
          {
            name: "Sheet1",
            data: agentResult.text
              .split("\n")
              .map((line) => [line]),
          },
        ];
      } else if (!workSheetsFromFile) {
        // Both paths failed — propagate error
        throw new Error(`All Excel extraction strategies failed for ${filename}`);
      }
      // else: keep the imperfect node-xlsx result
    }
