...
RUN yarn install --production --network-timeout 100000 && \
    yarn add prisma@5.3.1 --network-timeout 100000 && \
...
RUN mkdir -p /app/server/storage/documents /app/server/storage/vector-cache \
    /app/server/storage/direct-uploads /app/server/storage/lancedb \
    /app/collector/hotdir /app/collector/outputs

EXPOSE 3001

# Setup the healthcheck (long start-period: migrations + first boot)
